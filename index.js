...
// Настройка Socket.io: увеличили буфер для передачи картинок и разрешили CORS
const io = new Server(server, { 
    cors: { origin: "*" }, 
    maxHttpBufferSize: 1e7 
});

// Пути к базе данных (теперь точно в корне проекта)
...
// ВАЖНО: Раздаем файлы прямо из текущей папки (где лежат index.js и index.html)
...
io.on('connection', (socket) => {
    ...
    socket.on('accept_request', (sender) => {
        ...
        if (users[currentUser] && users[sender]) {
            ...
            if (!users[sender].friends.includes(sender) && !users[sender].friends.includes(currentUser)) {
...
// ГЛАВНОЕ ДЛЯ RENDER: слушаем порт, который дает система
const PORT = process.env.PORT || 10000;
server.listen(PORT, () => {
    console.log(`🚀 BURGEGRAM SERVER RUNNING ON PORT ${PORT}`);
});
